...
- **Single NAT Gateway (not one per AZ)**: A production system would typically duplicate NAT per AZ for full fault tolerance, but that doubles hourly NAT cost. For a learning/portfolio environment, one NAT is a deliberate cost-vs-availability tradeoff, not an oversight — if `public-subnet-a` or its AZ went down, private subnet egress would break until NAT was redeployed elsewhere. A production system handling real traffic would justify the extra cost; this one doesn't yet.
...
Rather than stopping once the setup worked, I deliberately broke this VPC in four ways to understand *why* each component matters, not just that it works: removing the `db-sg` inbound rule (security group chaining), removing the private route table's NAT route, detaching the Internet Gateway, and briefly exposing `db-sg` to `0.0.0.0/0` to reason through the risk before reverting it.

Full write-up with observed behavior and root-cause explanations: [TROUBLESHOOTING.md](./TROUBLESHOOTING.md)
